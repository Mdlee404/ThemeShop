...
├─ scripts/build_theme_index.py  # 自动解压 + 索引生成脚本
├─ public/
│  ├─ index.html                 # 主题商店首页
│  ├─ theme.html                 # 主题详情页
...
└─ tools/theme_builder_reference.py
```
...
```bash
python scripts/build_theme_index.py
```

执行后会：

1. 扫描 `packages/*.zip`
2. 解压到 `public/themes/<themeId>/`
3. 复制 ZIP 到 `public/downloads/`
4. 生成 `public/data/themes.json`
...
- 运行 `python scripts/build_theme_index.py`
- 若生成文件有变更，自动提交回仓库
...
- **Build command**: 留空（因为文件已在仓库中生成）
- **Build output directory**: `public`

> 若你希望 Cloudflare 侧也执行生成，可设置 Build command：
>
> ```bash
> python scripts/build_theme_index.py
> ```
>
> 并保证 Pages 环境有 Python（通常使用 GitHub Actions 预生成更稳）。
...
文件：`tools/theme_builder_reference.py`
...
- 构建逻辑集中在 `scripts/build_theme_index.py`
- 主题元数据统一来源：`public/data/themes.json`
- 后续可新增：
  - 主题评分/排序字段
  - 分类与筛选器（明暗风格、色系）
  - 可视化校验报告
